...
| Detectar HTTP Method Tampering | Baselines anônimo e autenticado, métodos alternativos, controle desconhecido e confirmação | Bypass anônimo, bypass do mesmo ator autenticado negado só num método e violação de contrato; handlers genéricos e métodos legítimos excluídos; bypass autenticado confirmado contra alvo real via `curl` e via `Agent.test_methods` com login real |
...
| Relatório válido com execução vazia | Objeto contendo apenas `findings: []` | App corrigida e ausência de configuração |
...
Os prazos e o envio por e-mail são responsabilidades operacionais do candidato. O agente não envia mensagens nem publica resultados. Cobertura de uma aplicação desconhecida não pode ser garantida; as limitações técnicas estão explicitadas no README. O JSON contém apenas findings, conforme o escopo do briefing.
