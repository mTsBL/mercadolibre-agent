...
Resultado: **41 testes passaram**; um teste adicional de inferência real fica desabilitado por padrão e foi executado separadamente. Os testes de integração usam Chromium real e servidores HTTP de loopback com rotas aleatórias. O stub do Ollama existe exclusivamente em `tests/fixtures.py` e não é importado pelo pacote `scanner`.
...
|---|---|---|---|---|---|---|---|
| SPA com bearer token | `completed` | `verified` | 4 | 16 | 10.413 / 563 | 116 | 227,281 s |
| Formulário com cookie e redirect | `completed` | `verified` | 4 | 15 | 8.748 / 523 | 115 | 181,670 s |
